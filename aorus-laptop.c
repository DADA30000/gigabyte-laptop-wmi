...
/* _SB_.PCI0.AMW0._WDG */
#define WMI_EVENT                                                              \
  "ABBC0F72-8EA1-11D1-00A0-C90629100000" // Hopefully, it's used for hotkeys
#define WMI_METHOD_WMBC                                                        \
  "ABBC0F6F-8EA1-11D1-00A0-C90629100000" // Seems to only return values
#define WMI_METHOD_WMBD                                                        \
  "ABBC0F75-8EA1-11D1-00A0-C90629100000" // Will probably do most of the work.
...
static int gigabyte_laptop_get_devstate2(u32 method_id, u32 arg2,
                                         void *result) {
...
static int gigabyte_laptop_hwmon_read(struct device *dev,
  ...
  case hwmon_pwm:
    switch (attr) {
    case hwmon_pwm_input:
      ret = gigabyte_laptop_get_devstate(fan_pwm_channels[channel], &output);
      if (ret)
        break;
      *val = output;
      break;
  ...
  if (fan_mode == FAN_FIXED_MODE || fan_mode == FAN_AUTO_MODE) {
    if (gigabyte->fan_mode < 3) { // If custom mode is off, enable it
      if (gigabyte->fan_mode > 0) {
        ret = gigabyte_laptop_set_devstate(fan_modes[gigabyte->fan_mode], 0,
                                           &result);
        if (ret)
          return ret;
      }
    ...
    if (gigabyte->fan_mode > 3) { // Fixed or auto mode active
      if (gigabyte->fan_mode == 4) {
        // Auto-maximum mode can only be turned off through gaming or silent
        // mode
        ret = gigabyte_laptop_set_devstate(FAN_GAMING_MODE, 0, &result);
        if (ret)
          return ret;
    ...
    if (fan_mode == FAN_AUTO_MODE) {
      ret = gigabyte_laptop_set_devstate(fan_mode, gigabyte->fan_custom_speed,
                                         &result);
      if (ret)
        return ret;
    } else {
    ...
    if (gigabyte->fan_mode >=
        3) { // Disable custom mode first. Will revert to normal mode.
...
static ssize_t fan_custom_speed_show(struct device *dev,
...
static ssize_t fan_custom_speed_store(struct device *dev,
  ...
  gigabyte = dev_get_drvdata(dev);
  // On some models, FAN_CUSTOM_SPEED will only set CPU fan speed.
  if (gigabyte->dual_fan_speed_enabled) {
    ret = gigabyte_laptop_set_devstate(GPU_FAN_DUTY, speed, &output);
    if (ret)
      return ret;
  }
...
static ssize_t gpu_boost_show(struct device *dev, struct device_attribute *attr,
                              char *buf) {
  struct gigabyte_laptop_wmi *gigabyte = dev_get_drvdata(dev);
...
static ssize_t fan_curve_index_show(struct device *dev,
                                    struct device_attribute *attr, char *buf) {
  struct gigabyte_laptop_wmi *gigabyte = dev_get_drvdata(dev);
...
static ssize_t fan_curve_index_store(struct device *dev,
                                     struct device_attribute *attr,
                                     const char *buf, size_t count) {
  int ret;
  unsigned int index;
...
static ssize_t fan_curve_data_show(struct device *dev,
                                   struct device_attribute *attr, char *buf) {
  struct gigabyte_laptop_wmi *gigabyte = dev_get_drvdata(dev);
...
static ssize_t fan_curve_data_store(struct device *dev,
                                    struct device_attribute *attr,
                                    const char *buf, size_t count) {
  int ret, output;
  u16 data;
  ...
  // payload = gigabyte->fan_curve.speed[gigabyte->fan_curve_index] << 16 |
  // gigabyte->fan_curve.temperature[gigabyte->fan_curve_index] << 8 | (u8)
  // gigabyte->fan_curve_index;
...
static ssize_t light_sensor_show(struct device *dev,
                                 struct device_attribute *attr, char *buf) {
  int ret, output_old;
  u8 output[4];
  struct gigabyte_laptop_wmi *gigabyte = dev_get_drvdata(dev);
  ...
  if (gigabyte->light_sensor_method == LIGHT_SENSOR_NEW)
    ...
    ret = gigabyte_laptop_get_devstate(gigabyte->light_sensor_method,
                                       &output_old);
  if (ret)
    return ret;
  if (gigabyte->light_sensor_method == LIGHT_SENSOR_NEW)
...
static ssize_t debug_method_store(struct device *dev,
                                  struct device_attribute *attr,
                                  const char *buf, size_t count) {
  int ret;
  u8 data;
...
static ssize_t debug_method_show(struct device *dev,
                                 struct device_attribute *attr, char *buf) {
  int ret, output;
  struct gigabyte_laptop_wmi *gigabyte = dev_get_drvdata(dev);
...
#define TOGGLE_DEVICE(_device, _id)                                            \
  static ssize_t _device##_toggle_show(                                        \
...
static int gigabyte_laptop_probe(struct device *dev) {
  ...
  if (ret)
    ...
    // Auto-maximum mode can't be read through WMI, so read EC register
    // containing it
...
obtain_custom_fan_speed:
  ...
  /*
          Newer models (VB and later) can have their fan speeds changed
     separately. We should check if the model we are using supports this, and if
     not, we will simply fall back to the old way of setting the fan speed.
  */
...
static int __init gigabyte_laptop_init(void) {
  ...
  gigabyte->hwmon_dev = hwmon_device_register_with_info(
      &gigabyte->pdev->dev, GIGABYTE_LAPTOP_FILE, gigabyte,
      &gigabyte_laptop_chip_info, NULL);
